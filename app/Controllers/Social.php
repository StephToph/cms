...
class Social extends BaseController {
	...
	public function post($param1='', $param2='', $param3='') {
		// check session login
		...
		if($param1 == 'manage') {
			...
			if($param2 == 'delete') {
					...
					if ($param3) {
						...
						if (!empty($edit)) {
							foreach ($edit as $e) {
								...
								$data['e_media_urls']    = $e->media_url;
								$data['e_platforms']    = json_decode($e->platforms, true);
								$data['e_status']       = $e->status;
								$data['e_scheduled_at'] = $e->scheduled_at;
								$data['e_posted_at']    = $e->posted_at;
							}
						}
					}
				}
				...
				if ($this->request->getMethod() == 'post') {
					...
					$media_url   = $this->request->getVar('media'); // fallback
					$platforms   = json_encode($this->request->getVar('platforms'));
					$scheduled_date   = $this->request->getVar('schedule_date');
					$scheduled_time   = $this->request->getVar('schedule_time');
					$scheduled = $scheduled_date.' '.$scheduled_time;
					$status      = 'scheduled'; // default
					
					
					// Array to collect uploaded media paths
					$media_urls = [];
					...
					if ($this->request->getFiles()) {
						...
						foreach ($files as $file) {
							if ($file->isValid() && !$file->hasMoved()) {
								$path = 'assets/images/social_media/' . $church_id . '/';
								$getFile = $this->Crud->file_upload($path, $file);

								if (!empty($getFile->path)) {
									$media_urls[] = $getFile->path;
								}
							}
						}
					}

					// Merge with any existing media if updating
					$existing = json_decode($this->request->getVar('existing_media'), true);
					if (!empty($existing) && is_array($existing)) {
						$media_urls = array_merge($existing, $media_urls);
					}

					// Check if any media was uploaded
					if (empty($media_urls)) {
						echo $this->Crud->msg('warning', 'You must upload at least one media file to continue.');
						die;
					}

					
					// Set the insert/update fields
					$ins_data = [
						'church_id'    => $church_id,
						'ministry_id'    => $ministry_id,
						'title'        => $title,
						'content'      => $content,
						'media_url'    => json_encode($media_urls),
						'platforms'    => $platforms,
						'scheduled_at' => date('Y-m-d h:iA', strtotime($scheduled)),
						'status'       => $status,
					];
				
					// Update or Create
					if ($post_id) {
						$updated = $this->Crud->updates('id', $post_id, $table, $ins_data);
						if ($updated > 0) {
							$who = $this->Crud->read_field('id', $church_id, 'church', 'name');
							$code = $this->Crud->read_field('id', $post_id, $table, 'title');
							$action = "$who updated Social Post ($code)";
							$this->Crud->activity('social_post', $post_id, $action);
				
							echo $this->Crud->msg('success', 'Post Updated Successfully');
							echo '<script>location.reload(false);</script>';
						} else {
							echo $this->Crud->msg('info', 'No Changes Made');
						}
					} else {
						if ($this->Crud->check2('title', $title, 'scheduled_at', $scheduled, $table) > 0) {
							echo $this->Crud->msg('warning', 'A post with this title is already scheduled for this date.');
						} else {
							$ins_data['reg_date'] = date('Y-m-d H:i:s');
							$ins_rec = $this->Crud->create($table, $ins_data);
				
							if ($ins_rec > 0) {
								$who = $this->Crud->read_field('id', $church_id, 'church', 'name');
								$code = $this->Crud->read_field('id', $ins_rec, $table, 'title');
								$action = "$who created Social Post ($code)";
								$this->Crud->activity('social_post', $ins_rec, $action);
				
								echo $this->Crud->msg('success', 'Post Created Successfully');
								echo '<script>location.reload(false);</script>';
							} else {
								echo $this->Crud->msg('danger', 'Something went wrong. Please try again.');
							}
						}
					}
		...
		if($param1 == 'load') {
			...
			if(!$log_id) {
				...
				$all_rec = $this->Crud->filter_social('', '', $log_id, $search, $switch_id);
                // $all_rec = json_decode($all_rec);
				if(!empty($all_rec)) { $counts = count($all_rec); } else { $counts = 0; }

				$query = $this->Crud->filter_social($limit, $offset, $log_id, $search, $switch_id);
				$data['count'] = $counts;
				...
				if (!empty($query)) {
					foreach ($query as $q) {
						$id = $q->id;
						$title = $q->title;
						$content = word_limiter(strip_tags($q->content), 10);
						$platforms = json_decode($q->platforms, true);
						$scheduled = !empty($q->scheduled_at) ? date('d/m/Y H:i', strtotime($q->scheduled_at)) : '-';
						$status = ucfirst($q->status);
						$media = json_decode($q->media_url, true);
						$church_id = $q->church_id;
						$author = $this->Crud->read_field('id', $q->church_id, 'church', 'name');
				
						// Media Preview (first file only)
						$thumb = '';
						if (!empty($media) && is_array($media)) {
							$first = $media[0];
							$isVideo = preg_match('/\.(mp4|avi|mov|wmv)$/i', $first);
							if ($isVideo) {
								$thumb = '<video src="' . site_url($first) . '" width="60" height="40" controls></video>';
							} else {
								$thumb = '<img src="' . site_url($first) . '" width="60" height="40">';
							}
						} else {
							$thumb = '<span class="badge bg-light text-muted">No Media</span>';
						}

						$all_btn = '<li><a href="javascript:;" class="text-success" onclick="preview('.$id.')"><em class="icon ni ni-eye"></em><span>Preview</span></a></li>';
						...
						if ($role_u != 1) {
							...
							$all_btn .= '
								<li><a href="javascript:;" class="text-primary pop" pageTitle="Edit Post" pageName="' . site_url($mod . '/manage/edit/' . $id) . '"><em class="icon ni ni-edit-alt"></em><span>Edit</span></a></li>
								<li><a href="javascript:;" class="text-danger pop" pageTitle="Delete Post" pageName="' . site_url($mod . '/manage/delete/' . $id) . '"><em class="icon ni ni-trash-alt"></em><span>Delete</span></a></li>
							';
						}
						...
						if (!empty($platforms)) {
							foreach ($platforms as $p) {
								$platform_badges .= '<span class="badge bg-secondary me-1">' . ucfirst($p) . '</span>';
							}
						}
			...
			if(empty($item)) {
				$resp['item'] = $items.'
					<tr><td colspan="8"><div class="text-center text-muted">
						<br/><br/><br/>
						<i class="ni ni-server" style="font-size:150px;"></i><br/><br/>'.translate_phrase('No Social Post Returned').'
					</div></td></tr>
				';
			} else {
		...
		if($param1 == 'manage') { // view for form data posting
			...
			$data['title'] = translate_phrase('Social Post').' - '.app_name;
								...
								<tr>
									...
									<td>
										<div class="btn-group">
											...
											<a href="javascript:;" class="mx-1 btn btn-sm btn-outline-primary pop" 
											   pageTitle="Share Meeting Link" 
											   pageSize="modal-lg" 
											   pageName="' . site_url($mod . '/manage/time_link/' . $id . '/' . $formatted_date . '/' . $record_key) . '">
												<em class="icon ni ni-share-alt"></em> <span>Share Link</span>
											</a>
										</div>
									</td>
								</tr>
