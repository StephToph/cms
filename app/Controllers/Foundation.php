...
class Foundation extends BaseController{
	...
	public function graduates($param1 = '', $param2 = '', $param3 = '')
	{
		// check session login
		if ($this->session->get('td_id') == '') {
			$request_uri = uri_string();
		...
		if ($param1 == 'load') {
			...
			if (!$log_id) {
				...
				if($role != 'developer' && $role != 'administrator'){
					if($church_id > 0){
						$members = $this->Crud->read3('is_member', 1, 'foundation_school', 2, 'church_id', $church_id, 'user');
						$visitors = $this->Crud->read2('foundation_school', 2, 'church_id', $church_id, 'visitors');
				
					} else {
						$members = $this->Crud->read3('is_member', 1, 'foundation_school', 2, 'ministry_id', $ministry_id, 'user');
						$visitors = $this->Crud->read2('foundation_school', 2, 'ministry_id', $ministry_id, 'visitors');
				
					}
				} else {
					$members = $this->Crud->read2('is_member', 1, 'foundation_school', 2, 'user');
					$visitors = $this->Crud->read_single('foundation_school', 2, 'visitors');
			...
			if (empty($item)) {
				$resp['item'] = '
					<tr><td colspan="8"><div class="text-center text-muted">
						<br/><br/><br/><br/>
						<i class="ni ni-users" style="font-size:150px;"></i><br/><br/>' . translate_phrase('No Graduate Returned') . '
					</div></td></tr>
				';
			} else {
				$resp['item'] = $item.'';
		...
		if ($param1 == 'enroll') { // view for form data posting
			...
			$data['title'] = translate_phrase('Foundation Graduate Student') . ' - ' . app_name;
			$data['page_active'] = $mod;
			return view($mod, $data);
		}
